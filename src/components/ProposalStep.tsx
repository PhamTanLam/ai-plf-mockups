 ...
 * Gom đầu ra thành hồ sơ trình khách; lối ra "Đã nhận đơn" (chốt đơn → sau nhận đơn).
 */
  ...
  return (
    <div className="max-w-3xl mx-auto bg-white border border-slate-200 rounded-2xl p-5 shadow-panel space-y-4">
      <div className="border-b border-slate-100 pb-3 flex items-center justify-between">
        <div>
          <h4 className="text-xs font-bold text-slate-500 font-mono uppercase tracking-wider">{t('ps.proposal.title')}</h4>
          <p className="text-[11px] text-slate-500 mt-0.5">{t('ps.proposal.subtitle')}</p>
        </div>
      </div>

      {gen.length === 0 ? (
        <div className="text-center text-xs text-slate-400 py-8 border border-dashed border-slate-200 rounded-xl">
          {t('ps.proposal.emptyOld')}
        </div>
      ) : (
              ...
              <div key={o.oid} className="group relative border border-slate-200/80 hover:border-brand-300 rounded-xl bg-white hover:bg-brand-50/5 transition-all duration-200 shadow-3xs hover:shadow-xs flex items-center gap-3 px-3.5 py-2.5">
                <div className="flex items-center gap-3 flex-1 min-w-0 cursor-pointer" onClick={() => setOpen(o)}>
                  ...
                  <span className="flex-1 min-w-0">
                    ...
                    <span className="block text-[10px] text-slate-450 mt-0.5">{t('ps.proposal.itemMeta')} · {fmtAgo(o.ts)}</span>
                  </span>
                </div>
      ...
      <button disabled={gen.length === 0} onClick={() => onToast?.(tf('ps.proposal.toastExport', { n: gen.length }))}
        ...
        <FileOutput className="w-3.5 h-3.5" /> {tf('ps.proposal.export', { n: gen.length })}
      </button>
      ...
      <div className="rounded-2xl border border-emerald-100 bg-gradient-to-r from-emerald-50/60 to-teal-50/30 p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 shadow-sm">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h5 className="text-xs font-extrabold text-slate-800 font-mono uppercase tracking-wider">{t('ps.proposal.acceptTitle')}</h5>
            <div className="mt-0.5"><MarkdownLite text={t('ps.proposal.acceptDesc2')} className="!text-[11px] !text-slate-650 [&_strong]:text-slate-800" /></div>
          </div>
        </div>
        <button
          onClick={onAccept}
        ...
        >
          <span>{t('ps.proposal.accept')}</span>
