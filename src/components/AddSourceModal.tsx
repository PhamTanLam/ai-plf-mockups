  ...
  return (
    <div className="fixed inset-0 z-50 bg-black/40 flex items-center justify-center p-6" onClick={e => { if (e.target === e.currentTarget) close() }}>
      ...
      <div className="bg-white rounded-2xl w-full max-w-[560px] shadow-pop overflow-hidden">
        {!paste ? (
          <>
            <div className="flex items-center px-5 py-3.5 border-b border-slate-200"><strong className="text-sm text-slate-800">Thêm nguồn</strong><button onClick={close} className="ml-auto text-slate-400 hover:text-slate-700 cursor-pointer"><X className="w-4 h-4" /></button></div>
            <div className="p-5">
              <div
                onClick={pickFiles}
                ...
                <div className="text-[15px] font-semibold text-slate-700 mt-2.5">Kéo-thả tệp của bạn vào đây</div>
                <div className="text-[11px] text-slate-400 mt-1">pdf · hình ảnh · tài liệu · âm thanh · video…</div>
              </div>
              <div className="flex justify-center gap-2.5 mt-4">
                <button onClick={pickFiles} className="inline-flex items-center gap-2 border border-slate-200 rounded-full px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 cursor-pointer font-medium"><Upload className="w-4 h-4" />Tải tệp lên</button>
                <button onClick={() => { setPaste(true); setText('') }} className="inline-flex items-center gap-2 border border-slate-200 rounded-full px-4 py-2.5 text-sm text-slate-700 hover:bg-slate-50 cursor-pointer font-medium"><Clipboard className="w-4 h-4" />Văn bản đã sao chép</button>
              </div>
            </div>
          </>
        ) : (
          <>
            <div className="flex items-center gap-2 px-5 py-3.5 border-b border-slate-200">
              ...
              <strong className="text-sm text-slate-800">Dán văn bản đã sao chép</strong>
